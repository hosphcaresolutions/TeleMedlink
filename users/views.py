...
def register(request):
  ...
  if request.method == 'POST':
    ...
    # Ensure a profile picture is optional
    ...
    if len(password) < 6:
      ...
      return render(request, 'users/register.html', context={'user_config': user_status, 'user_firstname': first_name, 'user_lastname': last_name, 'user_id': username, 'email': email, 'user_gender': gender})
    ...
    user = Users.objects.create_user(
      ...
      last_name=last_name,
      profile_avatar=profile_pic if profile_pic else "doctor/profiles/def.jpeg",
      username=username,
      email=email,
      gender=gender,
      birthday=birthday,
