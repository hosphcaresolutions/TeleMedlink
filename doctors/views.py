...
def doctor_dashboard(request):
    ...
    monthly_data = appointments.annotate(
        month=ExtractMonth('start_date')
    ).values('month').annotate(count=Count('id')).order_by('month')

    # Prepare data for status chart
    status_data = appointments.values('status__status').annotate(count=Count('id'))
    status_counts = Appointment.objects.filter(doctor=doctor).values('status__status').annotate(count=Count('id'))
...
def report_view(request):
    ...
    return render(request, 'doctors/report.html', {
        'doctors': doctors,
        'patients': patients
    })

# Report documents for doctors
def documents_view(request):
    appointments = Appointment.objects.filter(doctor__user=request.user)
    return render(request, 'doctors/documents.html', {'appointments': appointments})

def download_appointments_pdf(request):
    appointments = Appointment.objects.filter(doctor__user=request.user)
    template_path = 'doctors/appointments_pdf.html'
    context = {'appointments': appointments}
    response = HttpResponse(content_type='application/pdf')
    response['Content-Disposition'] = 'attachment; filename="doctor_appointments.pdf"'
